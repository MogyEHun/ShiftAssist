...
import { ShiftWithAssignee, User, ShiftStatus, CreateShiftPayload, ShiftTemplate, AvailabilityDate, Station } from '@/types'
...
export function ShiftDetailPanel({
  ...
  const STATUS_OPTIONS: { value: ShiftStatus; label: string }[] = [
    { value: 'published', label: t('shiftModal.statusPublished') },
    { value: 'draft', label: t('shiftModal.statusDraft') },
    { value: 'swappable', label: t('shiftModal.statusSwappable') },
  ]
  ...
  const [availWarning, setAvailWarning] = useState<{ status: 'unavailable' | 'partial'; from_time?: string | null; to_time?: string | null } | null>(null)
  ...
  useEffect(() => {
    if (!userId || !date || !availabilityDates) { setAvailWarning(null); return }
    const entry = availabilityDates.find(a => a.user_id === userId && a.date === date)
    if (entry?.status === 'unavailable' || entry?.status === 'partial') {
      setAvailWarning({ status: entry.status, from_time: entry.from_time, to_time: entry.to_time })
    } else {
      setAvailWarning(null)
    }
  }, [userId, date, availabilityDates])
  ...
  async function handleSave() {
    ...
    const payload: CreateShiftPayload = {
      ...
      status: plannerMode ? 'draft' : status,
      notes: notes || null,
  ...
  return (
    <>
      {/* Backdrop */}
      <div
        className={`fixed inset-0 bg-black/20 z-30 transition-opacity duration-300 ${visible ? 'opacity-100' : 'opacity-0'}`}
        onClick={handleClose}
      />

      {/* Slide-in panel */}
      <div
        className={`fixed inset-y-0 right-0 w-full md:w-[380px] bg-white z-40 flex flex-col shadow-2xl transition-transform duration-300 ease-out ${visible ? 'translate-x-0' : 'translate-x-full'}`}
      >
        {/* Header */}
        ...
        <div className="px-5 py-4 space-y-4 overflow-y-auto flex-1">
          ...
          {availWarning?.status === 'unavailable' && (
            <div className="flex items-start gap-2 bg-red-50 border border-red-300 text-red-800 text-sm px-3 py-2.5 rounded-lg">
              <AlertTriangle className="h-4 w-4 flex-shrink-0 mt-0.5 text-red-500" />
              <span>{t('shiftModal.unavailable')}</span>
            </div>
          )}

          {availWarning?.status === 'partial' && (
            <div className="flex items-start gap-2 bg-amber-50 border border-amber-200 text-amber-800 text-sm px-3 py-2.5 rounded-lg">
              <AlertTriangle className="h-4 w-4 flex-shrink-0 mt-0.5 text-amber-500" />
              <span>
                {t('shiftModal.partialAvailability')} {availWarning.from_time?.slice(0, 5)}–{availWarning.to_time?.slice(0, 5)}
              </span>
            </div>
          )}
        ...
        {/* Törlés megerősítés */}
        {confirmDelete && (
          <div className="mx-5 mb-4 bg-red-50 border border-red-200 rounded-lg p-3 flex-shrink-0">
        ...
        {/* Footer gombok */}
        <div className="flex items-center gap-3 px-5 py-4 border-t bg-gray-50 flex-shrink-0">
          ...
          <button
            onClick={handleSave}
            disabled={loading}
            className="px-5 py-2 bg-[#1a5c3a] text-white text-sm font-medium rounded-lg hover:bg-[#155033] transition-colors disabled:opacity-50 flex items-center gap-2"
          >
            {loading && (
