...
export function ShiftSwapResult({ recipientName, approved, shiftDate, managerNote, companyName }: Props) {
  return (
    <Html lang="hu">
      <Head />
      <Preview>Csereigény {approved ? 'jóváhagyva' : 'elutasítva'} – {shiftDate}</Preview>
        ...
        <Container style={{ maxWidth: '600px', margin: '0 auto', padding: '24px' }}>
          ...
          <Section style={{ background: '#ffffff', padding: '32px', border: '1px solid #e5e7eb', borderTop: 'none', borderRadius: '0 0 12px 12px' }}>
            <Heading as="h2" style={{ color: approved ? '#1a5c3a' : '#dc2626', marginTop: 0 }}>
              Csereigény {approved ? 'jóváhagyva ✓' : 'elutasítva ✗'}
            </Heading>
            <Text style={{ color: '#4b5563', lineHeight: '1.6' }}>Kedves {recipientName},</Text>
            <Text style={{ color: '#4b5563', lineHeight: '1.6' }}>
              A(z) <strong>{shiftDate}</strong> műszakra vonatkozó csereiény{' '}
            ...
            {managerNote && (
              <Section style={{ background: '#f9fafb', borderRadius: '8px', padding: '12px 16px', margin: '16px 0' }}>
                <Text style={{ color: '#4b5563', margin: 0, fontSize: '14px' }}>
                  <em>Vezető megjegyzése: {managerNote}</em>
                </Text>
              </Section>
            )}
...
export default ShiftSwapResult
