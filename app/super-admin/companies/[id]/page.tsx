...
import { CompanyDetailActions } from '@/components/super-admin/CompanyDetailActions'
...
const statusLabel: Record<string, string> = {
  active: 'Aktív', trialing: 'Trial', canceled: 'Lemondott', past_due: 'Lejárt',
}
const statusColor: Record<string, string> = {
  active: 'bg-green-100 text-green-800', trialing: 'bg-amber-100 text-amber-800',
  canceled: 'bg-red-100 text-red-800', past_due: 'bg-orange-100 text-orange-800',
}
...
export default async function SuperAdminCompanyDetailPage({ params }: Props) {
  ...
  const featureFlags = (company as any).feature_flags as Record<string, boolean> ?? {}
  const internalNotes = (company as any).internal_notes as string ?? ''

  return (
    <div className="p-6 max-w-5xl mx-auto space-y-6">
      <Link href="/super-admin/companies" className="inline-flex items-center gap-2 text-sm text-slate-500 hover:text-slate-700">
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
        ...
        <div className="mt-5 grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="font-medium text-slate-700">{new Date(company.created_at).toLocaleDateString('hu-HU')}</p>
          </div>
          <div>
            <p className="text-slate-400 text-xs mb-0.5">Trial vége</p>
            <p className="font-medium text-slate-700">
              {company.trial_ends_at ? new Date(company.trial_ends_at).toLocaleDateString('hu-HU') : '–'}
            </p>
          </div>
          <div>
        ...
        <div className="mt-5 pt-5 border-t border-slate-100 flex flex-wrap gap-3">
          <form action="/api/super-admin/impersonate" method="POST">
            <input type="hidden" name="companyId" value={company.id} />
            <input type="hidden" name="companyName" value={company.name} />
            <button type="submit" className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors">
      ...
      {/* Interaktív panelek (client component) */}
      <CompanyDetailActions
        companyId={company.id}
        currentPlan={(company.subscription_plan ?? 'basic') as 'basic' | 'premium'}
        currentStatus={status as 'active' | 'trialing' | 'canceled' | 'past_due'}
              ...
              {users.map((user) => (
                <tr key={user.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3">
                    {user.is_active ? (
                      <span className="inline-flex items-center gap-1 text-xs text-green-700"><CheckCircle className="h-3 w-3" /> Aktív</span>
                    ) : (
                      <span className="inline-flex items-center gap-1 text-xs text-red-600"><XCircle className="h-3 w-3" /> Inaktív</span>
                    )}
                  </td>
                </tr>
              ))}
              {users.length === 0 && (
                <tr><td colSpan={5} className="px-6 py-8 text-center text-slate-400">Nincs regisztrált felhasználó.</td></tr>
              )}
            </tbody>
          </table>
        </div>
